...
COPY sources/executables ./executables
COPY sources/external ./external
COPY sources/Cmago_container.toml ./Cmago.toml
RUN cmago update
ENV CC=clang
ENV CXX=clang++
ENV CMAKE_CXX_FLAGS="-fno-rtti -fno-exceptions -flto -ffunction-sections -fdata-sections"
...
ENV MAIN_PATH=build/$MAIN_NAME
RUN cp "$MAIN_PATH" "$TARGET_DIR"
RUN ldd "$MAIN_PATH" | grep "=>" | awk '{print $3}' | while read lib; do \
      if [ -f "$lib" ]; then \

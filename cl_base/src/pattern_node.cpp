...
bool PatternNode::valid(const Timer& timer) const{
    if(!size.valid()){
        timer.print(Color::RED, "PatternNode::valid : 节点尺寸不合法: ",size.to_string(),"\n");
        return false;
    }
    ...
    if(childs_size.size!=size.size){
        timer.print(Color::RED, "PatternNode::valid : 子节点尺寸和不等于父节点尺寸: 父节点 ",size.to_string(),", 子节点合并后 ",childs_size.to_string(),"\n");
        return false;
    }
    for(auto child:childs){
